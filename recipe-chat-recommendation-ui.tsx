...
const UnifiedRecipeChatUI = () => {
  ...
  const handleSend = async () => {
    ...
    const botResponse = "Based on your request, I recommend trying a delicious pasta primavera recipe. It's light and fresh, perfect for summer. Would you like more details or another suggestion?";
          ...
          {loading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {searchResults.map((recipe) => (
                <Card key={recipe.id}>
                  <img src={recipe.image || "/placeholder-image.jpg"} alt={recipe.title} className="w-full h-48 object-cover rounded-t-lg" />
                  <CardContent className="p-4">
                    <h3 className="text-xl font-semibold mb-2">{recipe.title}</h3>
                    <p className="text-gray-600 mb-2">{recipe.description}</p>
                    <div className="flex items-center mb-2">
                      <Clock className="mr-2 text-gray-500" size={16} />
                      <span className="text-sm text-gray-700">{recipe.cooking_time} minutes</span>
                    </div>
                    <div className="flex items-center mb-2">
                      <Utensils className="mr-2 text-gray-500" size={16} />
                      <span className="text-sm text-gray-700">{recipe.servings} servings</span>
                    </div>
                    <div className="flex justify-between items-center">
                      ...
                      <Button onClick={() => {/* Handle recipe click */}}>See Recipe</Button>
                    </div>
                  </CardContent>
                </Card>
              ))}
            ...
            <p className="text-center text-gray-600">No results found. Try adjusting your filters or ask for a recommendation in the chat.</p>
          )}
        </div>
      </CardContent>
    </Card>
