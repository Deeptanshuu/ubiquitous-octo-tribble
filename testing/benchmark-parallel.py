...
# Function to calculate weighted similarities for a chunk of recipes
def calculate_chunk_similarities(args):
    chunk, user_vector, user_cuisine, user_course = args
    chunk_vectors = vectorizer.transform(chunk['ingredients_text'])
    similarities = cosine_similarity(user_vector, chunk_vectors).flatten()
    weighted_similarities = [
        calculate_weighted_similarity(sim, recipe, user_cuisine, user_course)
        for sim, recipe in zip(similarities, chunk.to_dict('records'))
    ]
    return weighted_similarities

# Parallel version of the original recommendation function
@lru_cache(maxsize=None)
def get_recommendations_original(user_ingredients, user_cuisine, user_course, user_veg):
    ...
    # Split the filtered recipes into chunks
    num_cores = multiprocessing.cpu_count()
    chunks = np.array_split(filtered_recipes, num_cores)

    # Prepare arguments for multiprocessing
    chunk_args = [(chunk, user_vector, user_cuisine, user_course) for chunk in chunks]

    # Use multiprocessing to calculate weighted similarities
    with multiprocessing.Pool(processes=num_cores) as pool:
        chunk_similarities = pool.map(calculate_chunk_similarities, chunk_args)

    # Flatten the list of weighted similarities
    weighted_similarities = [item for sublist in chunk_similarities for item in sublist]
