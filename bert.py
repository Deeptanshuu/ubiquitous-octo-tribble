...
# Calculate BERT embeddings for all recipes
recipe_embeddings = np.array([get_bert_embedding(text) for text in recipes_data["ingredients_text"]])
...
def recommend_recipes():
    ...
    # Get BERT embedding for user input
    user_ingredients_text = " ".join(user_ingredients)
    user_embedding = get_bert_embedding(user_ingredients_text)

    # Calculate cosine similarity
    similarities = cosine_similarity([user_embedding], recipe_embeddings[filtered_recipes.index]).flatten()
